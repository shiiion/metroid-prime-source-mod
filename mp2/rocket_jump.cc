...
void on_missile_explosion(CEnergyProjectile* projectile, float dt, CStateManager& mgr) {
   ...
   CMRay test_ray(
       explosion_origin,
       player_origin + vec3(0, 0, player->get_fpbounds_z() - CTweakPlayer::instance()->get_eye_offset()));
...
void hooked_energyprojectile_think(CEnergyProjectile* projectile, float dt, CStateManager& mgr) {
   ...
   // call original think
   call_class_func<void, float, CStateManager&>(0x8016901c, projectile, dt, mgr);
